    ...
    def send_config(c: types.CallbackQuery):
        ...
        back_button = types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton(
                _('gl_back'), callback_data='config_loader'
            )
        )

        if not os.path.exists(path):
            bot.answer_callback_query(
                c.id, _('cfg_not_found_err', path), show_alert=True
            )
            return
        ...
        with open(path, 'r', encoding='utf-8') as f:
            ...
            if not data:
                bot.answer_callback_query(
                    c.id, _('cfg_empty_err', path), show_alert=True
                )
                return
            f.seek(0)
            bot.send_document(
                c.message.chat.id, f, caption=text, reply_markup=back_button
            )

        logger.info(
            _('log_cfg_downloaded', c.from_user.username, c.from_user.id, path)
        )
        bot.answer_callback_query(c.id)

    tg.cbq_handler(open_config_loader, lambda c: c.data == 'config_loader')
    tg.cbq_handler(
        send_config, lambda c: c.data.startswith(f'{CBT.DOWNLOAD_CFG}:')
    )
