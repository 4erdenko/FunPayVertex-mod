...
def init_auto_delivery_cp(crd: Vertex, *args):
    ...
    def check_ad_lot_exists(
        index: int, message_obj: Message, reply_mode: bool = True
    ) -> bool:
        """
        ...
        if index > len(crd.AD_CFG.sections()) - 1:
            update_button = K().add(
                B(_('gl_refresh'), callback_data=f'{CBT.AD_LOTS_LIST}:0')
            )
            if reply_mode:
                bot.reply_to(
                    message_obj,
                    _('ad_lot_not_found_err', index),
    ...
    def open_fp_lots_list(c: CallbackQuery):
        ...
        bot.edit_message_text(
            _(
                'desc_ad_fp_lot_list',
                crd.last_telegram_lots_update.strftime('%d.%m.%Y %H:%M:%S'),
            ),
            c.message.chat.id,
            c.message.id,
            reply_markup=kb.funpay_lots_list(crd, offset),
        )
        bot.answer_callback_query(c.id)
    ...
    def add_lot_manually(m: Message):
        ...
        """
        fp_lots_offset = tg.get_state(m.chat.id, m.from_user.id)['data'][
            'offset'
        ]
        ...
        if lot in crd.AD_CFG.sections():
            ...
            bot.reply_to(
                m,
                _('ad_lot_already_exists', utils.escape(lot)),
...
$product""",
        ...
        logger.info(
            _('log_ad_linked', m.from_user.username, m.from_user.id, lot)
        )
        ...
        bot.send_message(
            m.chat.id, _('ad_lot_linked', lot), reply_markup=keyboard
        )
    ...
    def create_gf(m: Message):
        ...
        if os.path.exists(f'storage/products/{file_name}'):
            ...
            bot.reply_to(
                m, _('gf_already_exists_err', file_name), reply_markup=keyboard
            )
            return
        ...
        try:
            ...
            bot.reply_to(
                m, _('gf_creation_err', file_name), reply_markup=error_keyboard
            )
        ...
        bot.send_message(
            m.chat.id, _('gf_created', file_name), reply_markup=keyboard
        )
    ...
    def edit_delivery_text(m: Message):
        ...
        if (
            ...
            bot.reply_to(
                m,
                _('ad_product_var_err', utils.escape(lot)),
    ...
    def act_link_gf(c: CallbackQuery):
        ...
        lot_index, offset = int(split[1]), int(split[2])
        result = bot.send_message(
            c.message.chat.id, _('ad_link_gf'), reply_markup=CLEAR_STATE_BTN()
        )
        tg.set_state(
            c.message.chat.id,
            result.id,
    ...
    def link_gf(m: Message):
        ...
        if file_name == '-':
            ...
            crd.save_config(crd.AD_CFG, 'configs/auto_delivery.cfg')
            logger.info(
                _('log_gf_unlinked', m.from_user.username, m.from_user.id, lot)
            )
            bot.reply_to(
                m,
                _('ad_gf_unlinked', utils.escape(lot)),
                reply_markup=keyboard,
            )
            return
        ...
        if not os.path.exists(f'storage/products/{file_name}'):
            ...
            try:
                with open(
                    f'storage/products/{file_name}', 'w', encoding='utf-8'
                ):
                    pass
            except:
                logger.debug('TRACEBACK', exc_info=True)
                bot.reply_to(
                    m, _('gf_creation_err', file_name), reply_markup=keyboard
                )
                return
        ...
        if exists:
            ...
            bot.reply_to(
                m,
                _('ad_gf_linked', file_name, utils.escape(lot)),
                reply_markup=keyboard,
            )
        else:
            logger.info(
            ...
            bot.reply_to(
                m,
                _('ad_gf_created_and_linked', file_name, utils.escape(lot)),
                reply_markup=keyboard,
            )
    ...
    def create_lot_delivery_test(c: CallbackQuery):
        ...
        bot.send_message(
            c.message.chat.id,
            _('test_ad_key_created', utils.escape(lot_name), key),
            reply_markup=keyboard,
        )
        bot.answer_callback_query(c.id)
    ...
    def del_lot(c: CallbackQuery):
        ...
        logger.info(
            _('log_ad_deleted', c.from_user.username, c.from_user.id, lot)
        )
        bot.edit_message_text(
            _('desc_ad_list'),
            c.message.chat.id,
    ...
    def update_funpay_lots_list(c: CallbackQuery):
        ...
        new_msg = bot.send_message(
            c.message.chat.id, _('ad_updating_lots_list')
        )
        bot.answer_callback_query(c.id)
        result = crd.update_lots_and_categories()
        if not result:
            bot.edit_message_text(
                _('ad_lots_list_updating_err'), new_msg.chat.id, new_msg.id
            )
            return
    ...
    def add_ad_to_lot(c: CallbackQuery):
        ...
        if fp_lot_index > len(crd.tg_profile.get_common_lots()) - 1:
            update_button = K().add(
                B(_('gl_refresh'), callback_data=f'{CBT.FP_LOTS_LIST}:0')
            )
            bot.edit_message_text(
                _('ad_lot_not_found_err', fp_lot_index),
                c.message.chat.id,
                c.message.id,
                reply_markup=update_button,
            )
        ...
        if lot.title in crd.AD_CFG.sections():
            ...
            bot.send_message(
                c.message.chat.id,
                _('ad_already_ad_err', utils.escape(lot.title)),
                reply_markup=keyboard,
            )
            bot.answer_callback_query(c.id)
            return
        ...
        ad_lots_offset = utils.get_offset(
            ad_lot_index, MENU_CFG.AD_BTNS_AMOUNT
        )
        keyboard = K().row(
            B(
                _('gl_back'),
        ...
        logger.info(
            _('log_ad_linked', c.from_user.username, c.from_user.id, lot.title)
        )

        bot.send_message(
            c.message.chat.id,
            _('ad_lot_linked', utils.escape(lot.title)),
            reply_markup=keyboard,
        )
        bot.answer_callback_query(c.id)
    ...
    def open_gf_settings(c: CallbackQuery):
        ...
        files = [
            i for i in os.listdir('storage/products') if i.endswith('.txt')
        ]
        if not check_products_file_exists(
        ...
        products_amount = vertex_tools.count_products(
            f'storage/products/{file_name}'
        )
        nl = '\n'
        delivery_objs = [
            i
            for i in crd.AD_CFG.sections()
            if crd.AD_CFG[i].get('productsFileName') == file_name
        ]
...
<i>{_('gl_last_update')}:</i>  <code>{datetime.datetime.now().strftime('%H:%M:%S')}</code>"""
    ...
    def add_products_to_file(m: Message):
        ...
        products = list(
            itertools.filterfalse(
                lambda el: not el, m.text.strip().split('\n')
            )
        )
        ...
        try:
            with open(
                f'storage/products/{file_name}', 'a', encoding='utf-8'
            ) as f:
                f.write('\n')
        ...
        bot.reply_to(
            m,
            _('gf_new_goods', len(products), file_name),
            reply_markup=keyboard,
        )
        ...
        with open(f'storage/products/{file_name}', 'r', encoding='utf-8') as f:
            ...
            if not data:
                bot.answer_callback_query(
                    c.id, _('gf_empty_error', file_name), show_alert=True
                )
                return
    ...
    # Основное меню настроек автовыдачи.
    tg.cbq_handler(
        open_ad_lots_list, lambda c: c.data.startswith(f'{CBT.AD_LOTS_LIST}:')
    )
    tg.cbq_handler(
        open_fp_lots_list, lambda c: c.data.startswith(f'{CBT.FP_LOTS_LIST}:')
    )
    tg.cbq_handler(
        act_add_lot_manually,
    ...
    # Меню настройки лотов.
    tg.cbq_handler(
        open_edit_lot_cp, lambda c: c.data.startswith(f'{CBT.EDIT_AD_LOT}:')
    )

    tg.cbq_handler(
        act_edit_delivery_text,
    ...
    tg.cbq_handler(
        act_link_gf, lambda c: c.data.startswith(f'{CBT.BIND_PRODUCTS_FILE}:')
    )
    tg.msg_handler(
        link_gf,
        func=lambda m: tg.check_state(
    ...
    tg.cbq_handler(
        switch_lot_setting, lambda c: c.data.startswith('switch_lot:')
    )
    tg.cbq_handler(
        create_lot_delivery_test,
    ...
    # Меню добавления лота с FunPay
    tg.cbq_handler(
        add_ad_to_lot, lambda c: c.data.startswith(f'{CBT.ADD_AD_TO_LOT}:')
    )
    tg.cbq_handler(
        update_funpay_lots_list,
