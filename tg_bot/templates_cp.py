...
def init_templates_cp(vertex: Vertex, *args):
    ...
    def check_template_exists(
        template_index: int, message_obj: Message
    ) -> bool:
        """
        ...
        if template_index > len(vertex.telegram.answer_templates) - 1:
            update_button = K().add(
                B(_('gl_refresh'), callback_data=f'{CBT.TMPLT_LIST}:0')
            )
            bot.edit_message_text(
                _('tmplt_not_found_err', template_index),
                message_obj.chat.id,
                message_obj.id,
    ...
    def open_templates_list_in_ans_mode(c: CallbackQuery):
        ...
        bot.edit_message_reply_markup(
            c.message.chat.id,
            c.message.id,
            reply_markup=keyboards.templates_list_ans_mode(
                vertex, offset, node_id, username, prev_page, extra
            ),
        )
    ...
    def add_template(m: Message):
        ...
        if template in tg.answer_templates:
            error_keyboard = K().row(
                ...
                B(
                    _('tmplt_add_another'),
                    callback_data=f'{CBT.ADD_TMPLT}:{offset}',
                ),
            )
            bot.reply_to(
                m, _('tmplt_already_exists_err'), reply_markup=error_keyboard
            )
            return
    ...
    def send_template(c: CallbackQuery):
        ...
        if template_index > len(tg.answer_templates) - 1:
            bot.send_message(
                c.message.chat.id, _('tmplt_not_found_err', template_index)
            )
            if prev_page == 0:
                bot.edit_message_reply_markup(
                    c.message.chat.id,
                ...
                bot.edit_message_reply_markup(
                    c.message.chat.id,
                    c.message.id,
                    reply_markup=keyboards.new_order(
                        extra[0],
                        username,
                        node_id,
        ...
        text = tg.answer_templates[template_index].replace(
            '$username', username
        )
        ...
        if result:
            bot.send_message(
                c.message.chat.id,
                _('tmplt_msg_sent', node_id, username, utils.escape(text)),
    ...
    tg.cbq_handler(
        open_templates_list, lambda c: c.data.startswith(f'{CBT.TMPLT_LIST}:')
    )
    tg.cbq_handler(
        open_templates_list_in_ans_mode,
        lambda c: c.data.startswith(f'{CBT.TMPLT_LIST_ANS_MODE}:'),
    )
    tg.cbq_handler(
        open_edit_template_cp,
        lambda c: c.data.startswith(f'{CBT.EDIT_TMPLT}:'),
    )
    tg.cbq_handler(
        act_add_template, lambda c: c.data.startswith(f'{CBT.ADD_TMPLT}:')
    )
    tg.msg_handler(
        add_template,
        func=lambda m: tg.check_state(
            m.chat.id, m.from_user.id, CBT.ADD_TMPLT
        ),
    )
    tg.cbq_handler(
        del_template, lambda c: c.data.startswith(f'{CBT.DEL_TMPLT}:')
    )
    tg.cbq_handler(
        send_template, lambda c: c.data.startswith(f'{CBT.SEND_TMPLT}:')
    )
