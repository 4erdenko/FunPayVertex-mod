...
def check_file(tg: TGBot, msg: types.Message) -> bool:
    ...
    if msg.document.file_size >= 20971520:
        tg.bot.send_message(
            msg.chat.id, '❌ Размер файла не должен превышать 20МБ.'
        )
        return False
    return True
...
def download_file(
    ...
    try:
        ...
        tg.bot.send_message(
            msg.chat.id, '❌ Произошла ошибка при загрузке файла.'
        )
        logger.debug('TRACEBACK', exc_info=True)
        return False

    path = (
        f'storage/cache/{file_name}'
        if not custom_path
        else os.path.join(custom_path, file_name)
    )
...
def init_uploader(vertex: Vertex):
    ...
    def upload_products_file(m: types.Message):
        ...
        try:
            products_count = vertex_tools.count_products(
                f'storage/products/{utils.escape(m.document.file_name)}'
            )
        except:
            bot.send_message(
                m.chat.id, '❌ Произошла ошибка при подсчете товаров.'
            )
            logger.debug('TRACEBACK', exc_info=True)
            return

        file_number = os.listdir('storage/products').index(
        ...
        logger.info(
            f'Пользователь $MAGENTA@{m.from_user.username} (id: {m.from_user.id})$RESET '
    ...
    def act_upload_main_config(c: types.CallbackQuery):
        result = bot.send_message(
            c.message.chat.id,
            'Отправьте мне основной конфиг.',
            reply_markup=CLEAR_STATE_BTN(),
        )
        tg.set_state(
            c.message.chat.id, result.id, c.from_user.id, 'upload_main_config'
        )
        bot.answer_callback_query(c.id)
    ...
    def upload_main_config(m: types.Message):
        ...
        try:
            new_config = cfg_loader.load_main_config(
                'storage/cache/temp_main.cfg'
            )
        except excs.ConfigParseError as e:
            bot.send_message(
                m.chat.id,
                f'❌ Произошла ошибка при обработке основного конфига: '
            ...
            bot.send_message(
                m.chat.id,
                '❌ Произошла ошибка при проверке конфига автовыдачи.',
            )
            logger.debug('TRACEBACK', exc_info=True)
            return
        ...
        bot.send_message(
            m.chat.id,
            '✅ Основной конфиг успешно загружен. \n'
    ...
    def act_upload_auto_response_config(c: types.CallbackQuery):
        result = bot.send_message(
            c.message.chat.id,
            'Отправьте мне конфиг автоответчика.',
            reply_markup=CLEAR_STATE_BTN(),
        )
        tg.set_state(
            c.message.chat.id,
    ...
    def upload_auto_response_config(m: types.Message):
        ...
        try:
            ...
            bot.send_message(
                m.chat.id,
                f'❌ Произошла ошибка при обработке конфига автоответчика: '
                f'<code>{utils.escape(str(e))}</code>',
            )
            return
        except UnicodeDecodeError:
            ...
            bot.send_message(
                m.chat.id,
                '❌ Произошла ошибка при проверке конфига автоответчика.',
            )
            logger.debug('TRACEBACK', exc_info=True)
            return
    ...
    def act_upload_auto_delivery_config(c: types.CallbackQuery):
        result = bot.send_message(
            c.message.chat.id,
            'Отправьте мне конфиг автовыдачи.',
            reply_markup=CLEAR_STATE_BTN(),
        )
        tg.set_state(
            c.message.chat.id,
    ...
    def upload_auto_delivery_config(m: types.Message):
        ...
        try:
            ...
            bot.send_message(
                m.chat.id,
                f'❌ Произошла ошибка при обработке конфига автовыдачи: '
                f'<code>{utils.escape(str(e))}</code>',
            )
            return
        except UnicodeDecodeError:
    ...
    def upload_plugin(m: types.Message):
        ...
        if not download_file(
            tg,
            m,
            f'{utils.escape(m.document.file_name)}',
        ...
        keyboard = types.InlineKeyboardMarkup().add(
            Button('◀️Назад', callback_data=f'{CBT.PLUGINS_LIST}:{offset}')
        )
        bot.send_message(
            m.chat.id,
    ...
    def send_funpay_image(m: types.Message):
        ...
        if not m.photo:
            tg.bot.send_message(
                m.chat.id,
                '❌ Поддерживаются только форматы <code>.png</code>, <code>.jpg</code>, '
                '<code>.gif</code>.',
            )
            return
        photo = m.photo[-1]
        if photo.file_size >= 20971520:
            tg.bot.send_message(
                m.chat.id, '❌ Размер файла не должен превышать 20МБ.'
            )
            return
        ...
        try:
            ...
            result = vertex.account.send_message(
                chat_id, None, username, image_id
            )
            if not result:
                tg.bot.reply_to(
                    m,
                    f'❌ Не удалось отправить сообщение в переписку '
                    ...
                    reply_markup=keyboards.reply(
                        chat_id, username, again=True
                    ),
                )
                return
            tg.bot.reply_to(
                m,
                f'✅ Сообщение отправлено в переписку '
                f'<a href="https://funpay.com/chat/?node={chat_id}">{username}</a>.',
    ...
    def upload_image(m: types.Message):
        ...
        try:
            ...
            tg.bot.reply_to(
                m,
                f'❌ Не удалось отправить выгрузить изображение. '
                f'Подробнее в файле <code>logs/log.log</code>',
            )
            return
        bot.reply_to(
    ...
    tg.cbq_handler(
        act_upload_main_config, lambda c: c.data == 'upload_main_config'
    )
