...
class TGBot:
    def __init__(self, vertex: Vertex):
        ...
        self.bot = telebot.TeleBot(
            self.vertex.MAIN_CFG['Telegram']['token'],
            parse_mode='HTML',
        ...
        self.attempts = (
            {}
        )  # {user_id: attempts} - попытки авторизации в Telegram ПУ.
        self.init_messages = (
            []
        )  # [(chat_id, message_id)] - список сообщений о запуске TG бота.
        ...
        self.notification_settings = (
            utils.load_notification_settings()
        )  # настройки уведомлений.
        self.answer_templates = (
            utils.load_answer_templates()
        )  # заготовки ответов.
    ...
    def is_notification_enabled(
        ...
        try:
            return bool(
                self.notification_settings[str(chat_id)][notification_type]
            )
        except KeyError:
            return False

    def toggle_notification(
        self, chat_id: int, notification_type: str
    ) -> bool:
        """
    ...
    def is_file_handler(self, m: Message):
        ...
        ) and m.content_type in ['photo', 'document']
    ...
    def run_file_handlers(self, m: Message):
        ...
        ) is None or state['state'] not in self.file_handlers:
            return
        try:
    ...
    def reg_admin(self, m: Message):
        ...
        """
        if m.chat.type != 'private' or (
            m.from_user.id in self.attempts
            and self.attempts.get(m.from_user.id) >= 5
        ):
            return
        if m.text == self.vertex.MAIN_CFG['Telegram']['secretKey']:
            ...
            logger.warning(
                _('log_access_granted', m.from_user.username, m.from_user.id)
            )
        else:
            self.attempts[m.from_user.id] = (
                self.attempts[m.from_user.id] + 1
                if m.from_user.id in self.attempts
                else 1
            )
            text = _('access_denied', m.from_user.username)
            logger.warning(
                _('log_access_attempt', m.from_user.username, m.from_user.id)
            )
    ...
    def send_profile(self, m: Message):
        ...
        try:
            ...
            self.bot.send_message(
                m.chat.id,
                utils.generate_profile_text(self.vertex),
                reply_markup=telebot.types.InlineKeyboardMarkup()
                .add(
                    telebot.types.InlineKeyboardButton(
                        '🔄 Обновить', callback_data='update_profile'
                    )
                )
                .add(
                    telebot.types.InlineKeyboardButton(
                        '▶️ Еще', callback_data='update_adv_profile'
                    )
                ),
            )
            ...
            self.bot.edit_message_text(
                _('profile_updating_error'), new_msg.chat.id, new_msg.id
            )
            logger.debug('TRACEBACK', exc_info=True)
    ...
    def update_profile(self, c: CallbackQuery):
        ...
        new_msg = self.bot.send_message(
            c.message.chat.id, _('updating_profile')
        )
        try:
            self.vertex.account.get()
            self.vertex.balance = self.vertex.get_balance()
    ...
    def update_adv_profile(self, c: CallbackQuery):
        ...
        try:
            ...
            self.vertex.balance = self.vertex.get_balance()
            self.bot.edit_message_text(
                utils.generate_adv_profile(self.vertex.account),
                c.message.chat.id,
                c.message.id,
                reply_markup=telebot.types.InlineKeyboardMarkup()
                .add(
                    telebot.types.InlineKeyboardButton(
                        '🔄 Обновить', callback_data='update_adv_profile'
                    )
                )
                .add(
                    telebot.types.InlineKeyboardButton(
                        '◀️ Назад', callback_data='update_profile'
                    )
                ),
            )
            self.bot.delete_message(new_msg.chat.id, new_msg.id)
    ...
    def send_orders(self, m: telebot.types.Message):
        new_mes = self.bot.send_message(
            m.chat.id, 'Сканирую заказы (это может занять какое-то время)...'
        )
        try:
        ...
        if not orders:
            self.bot.edit_message_text(
                '❌ Просроченных заказов нет.', new_mes.chat.id, new_mes.id
            )
            logger.debug('TRACEBACK', exc_info=True)
            return
        ...
        self.bot.edit_message_text(
            f'<code>{utils.escape(text)}</code>', new_mes.chat.id, new_mes.id
        )
    ...
    def act_manual_delivery_test(self, m: Message):
        ...
        result = self.bot.send_message(
            m.chat.id,
            _('create_test_ad_key'),
            reply_markup=skb.CLEAR_STATE_BTN(),
        )
        self.set_state(
            m.chat.id, result.id, m.from_user.id, CBT.MANUAL_AD_TEST
        )
    ...
    def manual_delivery_text(self, m: Message):
        ...
        self.bot.send_message(
            m.chat.id, _('test_ad_key_created', utils.escape(lot_name), key)
        )
    ...
    def act_ban(self, m: Message):
        ...
        """
        result = self.bot.send_message(
            m.chat.id, _('act_blacklist'), reply_markup=skb.CLEAR_STATE_BTN()
        )
    ...
    def ban(self, m: Message):
        ...
        if nickname in self.vertex.blacklist:
            self.bot.send_message(
                m.chat.id, _('already_blacklisted', nickname)
            )
            return
    ...
    def act_unban(self, m: Message):
        ...
        """
        result = self.bot.send_message(
            m.chat.id, _('act_unban'), reply_markup=skb.CLEAR_STATE_BTN()
        )
    ...
    def send_ban_list(self, m: Message):
        ...
        if not self.vertex.blacklist:
            ...
            return
        blacklist = ', '.join(
            f'<code>{i}</code>' for i in self.vertex.blacklist
        )
    ...
    def act_edit_watermark(self, m: Message):
        ...
        result = self.bot.send_message(
            m.chat.id,
            _('act_edit_watermark'),
            reply_markup=skb.CLEAR_STATE_BTN(),
        )
        self.set_state(
            m.chat.id, result.id, m.from_user.id, CBT.EDIT_WATERMARK
        )
    ...
    def send_system_info(self, m: Message):
        ...
        self.bot.send_message(
            ...
            _(
                ...
                vertex_tools.time_to_str(uptime),
                m.chat.id,
            ),
        )
    ...
    def ask_power_off(self, m: Message):
        ...
        self.bot.send_message(
            m.chat.id,
            _('power_off_0'),
            reply_markup=kb.power_off(self.vertex.instance_id, 0),
        )
    ...
    def cancel_power_off(self, c: CallbackQuery):
        ...
        """
        self.bot.edit_message_text(
            _('power_off_cancelled'), c.message.chat.id, c.message.id
        )
        self.bot.answer_callback_query(c.id)
    ...
    def power_off(self, c: CallbackQuery):
        ...
        if instance_id != self.vertex.instance_id:
            self.bot.edit_message_text(
                _('power_off_error'), c.message.chat.id, c.message.id
            )
            self.bot.answer_callback_query(c.id)
            return
        ...
        if state == 6:
            self.bot.edit_message_text(
                _('power_off_6'), c.message.chat.id, c.message.id
            )
            self.bot.answer_callback_query(c.id)
    ...
    def send_funpay_message(self, message: Message):
        ...
        if result:
            self.bot.reply_to(
                message,
                _('msg_sent', node_id, username),
                reply_markup=kb.reply(
                    node_id, username, again=True, extend=True
                ),
            )
        else:
            self.bot.reply_to(
    ...
    def act_upload_image(self, m: Message):
        ...
        """
        result = self.bot.send_message(
            m.chat.id, _('send_img'), reply_markup=skb.CLEAR_STATE_BTN()
        )
    ...
    def edit_greetings_text(self, m: Message):
        ...
        self.vertex.save_config(self.vertex.MAIN_CFG, 'configs/_main.cfg')
        keyboard = K().row(
            B(_('gl_back'), callback_data=f'{CBT.CATEGORY}:gr'),
    ...
    def edit_order_confirm_reply_text(self, m: Message):
        ...
        self.vertex.save_config(self.vertex.MAIN_CFG, 'configs/_main.cfg')
        keyboard = K().row(
            B(_('gl_back'), callback_data=f'{CBT.CATEGORY}:oc'),
    ...
    def act_edit_review_reply_text(self, c: CallbackQuery):
        ...
        text = (
            f"{_('v_edit_review_reply_text', '⭐' * stars)}\n\n{_('v_list')}:\n"
            + '\n'.join(_(i) for i in variables)
        )
        result = self.bot.send_message(
            c.message.chat.id, text, reply_markup=skb.CLEAR_STATE_BTN()
    ...
    def edit_review_reply_text(self, m: Message):
        ...
        self.vertex.save_config(self.vertex.MAIN_CFG, 'configs/_main.cfg')
        keyboard = K().row(
            B(_('gl_back'), callback_data=f'{CBT.CATEGORY}:rr'),
            B(
                _('gl_edit'),
        ...
        self.bot.reply_to(
            m, _('review_reply_changed', '⭐' * stars), reply_markup=keyboard
        )
    ...
    def extend_new_message_notification(self, c: CallbackQuery):
        ...
        for i in messages:
            ...
            msg_text = (
                f'<code>{i.text}</code>'
                if i.text
                else f"<a href=\"{i.image_link}\">{_('photo')}</a>"
            )
    ...
    def refund(self, c: CallbackQuery):
        ...
        while attempts:
            try:
                ...
                if not new_msg:
                    new_msg = self.bot.send_message(
                        c.message.chat.id,
                        _('refund_attempt', order_id, attempts),
                    )
                else:
                    self.bot.edit_message_text(
                        _('refund_attempt', order_id, attempts),
            ...
            self.bot.edit_message_text(
                _('refund_error', order_id), new_msg.chat.id, new_msg.id
            )
        ...
        if not new_msg:
            self.bot.send_message(
                c.message.chat.id, _('refund_complete', order_id)
            )
        else:
            self.bot.edit_message_text(
                _('refund_complete', order_id), new_msg.chat.id, new_msg.id
            )
    ...
    def open_order_menu(self, c: CallbackQuery):
        ...
        self.bot.edit_message_reply_markup(
            c.message.chat.id,
            c.message.id,
            reply_markup=kb.new_order(
                order_id, username, node_id, no_refund=no_refund
            ),
        )
    ...
    def switch_param(self, c: CallbackQuery):
        ...
        if section == 'FunPay' and option == 'oldMsgGetMode':
            ...
            self.vertex.MAIN_CFG[section][option] = str(
                int(not int(self.vertex.MAIN_CFG[section][option]))
            )
            self.vertex.save_config(self.vertex.MAIN_CFG, 'configs/_main.cfg')
    ...
    def switch_chat_notification(self, c: CallbackQuery):
        ...
        keyboard = (
            kb.announcements_settings
            if notification_type
            in [
                utils.NotificationTypes.announcement,
    ...
    def open_settings_section(self, c: CallbackQuery):
        ...
        sections = {
            ...
            'tg': (
                _('desc_ns', c.message.chat.id),
                kb.notifications_settings,
                [self.vertex, c.message.chat.id],
            ),
            ...
            'gr': (
                _(
                    'desc_gr',
                    utils.escape(
                        self.vertex.MAIN_CFG['Greetings']['greetingsText']
                    ),
                ),
            ...
            'oc': (
                _(
                    'desc_oc',
                    utils.escape(
                        self.vertex.MAIN_CFG['OrderConfirm']['replyText']
                    ),
                ),
    ...
    def cancel_action(self, call: CallbackQuery):
        ...
        """
        result = self.clear_state(
            call.message.chat.id, call.from_user.id, True
        )
        if result is None:
            self.bot.answer_callback_query(call.id)
    ...
    def param_disabled(self, c: CallbackQuery):
        ...
        self.bot.answer_callback_query(
            c.id, _('param_disabled'), show_alert=True
        )
    ...
    def send_announcements_kb(self, m: Message):
        ...
        self.bot.send_message(
            m.chat.id,
            _('desc_an'),
            reply_markup=kb.announcements_settings(self.vertex, m.chat.id),
        )
        ...
        if not text:
            ...
            self.bot.send_message(
                ...
                _(
                    'review_reply_text',
                    '⭐' * stars,
                    self.vertex.MAIN_CFG['ReviewReply'][
                        f'star{stars}ReplyText'
                    ],
                ),
                reply_markup=keyboard,
            )
    ...
    def switch_lang(self, c: CallbackQuery):
        ...
        if localizer.current_language == 'eng':
            self.bot.answer_callback_query(
                c.id,
                'The translation may be incomplete and contain errors.\n\n'
    ...
    def open_keyboard(self, m: Message):
        self.bot.send_message(
            m.chat.id, 'Клавиатура появилась!', reply_markup=skb.OLD_KEYBOARD
        )

    def close_keyboard(self, m: Message):
        self.bot.send_message(
            m.chat.id, 'Клавиатура скрыта!', reply_markup=ReplyKeyboardRemove()
        )
    ...
    def __register_handlers(self):
        ...
        """
        self.mdw_handler(
            self.setup_chat_notifications, update_types=['message']
        )
        self.msg_handler(
            self.reg_admin,
        ...
        self.cbq_handler(
            self.param_disabled,
            lambda c: c.data.startswith(CBT.PARAM_DISABLED),
        )
        self.msg_handler(
        ...
        self.cbq_handler(
            self.update_adv_profile, lambda c: c.data == 'update_adv_profile'
        )
        self.msg_handler(self.send_profile, commands=['profile'])
        self.msg_handler(self.send_orders, commands=['old_orders'])
        self.cbq_handler(
            self.update_profile, lambda c: c.data == CBT.UPDATE_PROFILE
        )
        ...
        self.cbq_handler(
            self.act_edit_greetings_text,
            lambda c: c.data == CBT.EDIT_GREETINGS_TEXT,
        )
        self.msg_handler(
        ...
        self.msg_handler(
            self.ban,
            func=lambda m: self.check_state(
                m.chat.id, m.from_user.id, CBT.BAN
            ),
        )
        ...
        self.msg_handler(
            self.unban,
            func=lambda m: self.check_state(
                m.chat.id, m.from_user.id, CBT.UNBAN
            ),
        )
        ...
        self.msg_handler(
            self.send_settings_menu, func=lambda m: m.text == '⚙️ Настройки ⚙️'
        )
        self.msg_handler(
            self.send_system_info, func=lambda m: m.text == '📈 Система 📈'
        )
        self.msg_handler(
            self.restart_vertex, func=lambda m: m.text == '🔄 Перезапуск 🔄'
        )
        self.msg_handler(
            self.close_keyboard, func=lambda m: m.text == '❌ Закрыть ❌'
        )
        self.msg_handler(
            self.ask_power_off, func=lambda m: m.text == '🔌 Отключение 🔌'
        )
        ...
        self.cbq_handler(
            self.cancel_refund,
            lambda c: c.data.startswith(f'{CBT.REFUND_CANCELLED}:'),
        )
        self.cbq_handler(
            self.refund,
            lambda c: c.data.startswith(f'{CBT.REFUND_CONFIRMED}:'),
        )
        self.cbq_handler(
        ...
        self.cbq_handler(
            ...
            lambda c: c.data.startswith(f'{CBT.CATEGORY}:'),
        )
        self.cbq_handler(
        ...
        self.cbq_handler(
            self.cancel_action, lambda c: c.data == CBT.CLEAR_STATE
        )
        self.cbq_handler(
        ...
        self.cbq_handler(
            self.switch_lang, lambda c: c.data.startswith(f'{CBT.LANG}:')
        )
    ...
    def setup_commands(self):
        ...
        """
        commands = [
            BotCommand(f'/{i}', self.commands[i]) for i in self.commands
        ]
    ...
    def run(self):
        ...
        try:
            ...
            self.bot.infinity_polling(
                logger_level=logging.DEBUG, skip_pending=True
            )
        except:
