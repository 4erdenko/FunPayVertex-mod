...
from tg_bot import CBT, MENU_CFG, keyboards, utils
...
def init_auto_response_cp(vertex: Vertex, *args):
    ...
    def check_command_exists(
        command_index: int, message_obj: Message, reply_mode: bool = True
    ) -> bool:
        """
        ...
        if command_index > len(vertex.RAW_AR_CFG.sections()) - 1:
            update_button = K().add(
                B(_('gl_refresh'), callback_data=f'{CBT.CMD_LIST}:0')
            )
            if reply_mode:
                bot.reply_to(
                    message_obj,
                    _('ar_cmd_not_found_err', command_index),
    ...
    def add_command(m: Message):
        ...
        for cmd in commands:
            if commands.count(cmd) > 1:
                bot.reply_to(
                    m,
                    _('ar_subcmd_duplicate_err', utils.escape(cmd)),
            ...
            if cmd in vertex.AR_CFG.sections():
                bot.reply_to(
                    m,
                    _('ar_cmd_already_exists_err', utils.escape(cmd)),
                    reply_markup=error_keyboard,
                )
                return

        vertex.RAW_AR_CFG.add_section(raw_command)
        vertex.RAW_AR_CFG.set(
            raw_command,
            'response',
            'Данной команде необходимо настроить текст ответа :(',
        )
        ...
        keyboard = K().row(
            ...
            B(
                _('gl_configure'),
                callback_data=f'{CBT.EDIT_CMD}:{command_index}:{offset}',
            ),
        )
        logger.info(
            _(
        ...
        bot.reply_to(
            m,
            _('ar_cmd_added', utils.escape(raw_command)),
    ...
    def open_edit_command_cp(c: CallbackQuery):
        ...
        if not check_command_exists(
            command_index, c.message, reply_mode=False
        ):
            bot.answer_callback_query(c.id)
            return
        ...
        notification_text = (
            ...
            else 'Пользователь $username ввел команду $message_text.'
        )
...
<i>{_('gl_last_update')}:</i>  <code>{datetime.datetime.now().strftime('%H:%M:%S')}</code>"""
        bot.edit_message_text(
            message, c.message.chat.id, c.message.id, reply_markup=keyboard
        )
        bot.answer_callback_query(c.id)
    ...
    def edit_command_response(m: Message):
        ...
        """
        command_index = tg.get_state(m.chat.id, m.from_user.id)['data'][
            'command_index'
        ]
        ...
        bot.reply_to(
            ...
            _(
                ...
                utils.escape(command),
                utils.escape(response_text),
            ),
            reply_markup=keyboard,
        )
    ...
    def act_edit_command_notification(c: CallbackQuery):
        ...
        tg.set_state(
            ...
            result.id,
            c.from_user.id,
            CBT.EDIT_CMD_NOTIFICATION_TEXT,
            {'command_index': command_index, 'offset': offset},
        )
        bot.answer_callback_query(c.id)
    ...
    def edit_command_notification(m: Message):
        ...
        bot.reply_to(
            ...
            _(
                ...
                utils.escape(command),
                utils.escape(notification_text),
            ),
            reply_markup=keyboard,
        )
    ...
    tg.cbq_handler(
        open_commands_list, lambda c: c.data.startswith(f'{CBT.CMD_LIST}:')
    )

    tg.cbq_handler(act_add_command, lambda c: c.data == CBT.ADD_CMD)
    tg.msg_handler(
        add_command,
        func=lambda m: tg.check_state(m.chat.id, m.from_user.id, CBT.ADD_CMD),
    )

    tg.cbq_handler(
        open_edit_command_cp, lambda c: c.data.startswith(f'{CBT.EDIT_CMD}:')
    )

    tg.cbq_handler(
        act_edit_command_response,
        lambda c: c.data.startswith(f'{CBT.EDIT_CMD_RESPONSE_TEXT}:'),
    )
    tg.msg_handler(
        edit_command_response,
        func=lambda m: tg.check_state(
            m.chat.id, m.from_user.id, CBT.EDIT_CMD_RESPONSE_TEXT
        ),
    )

    tg.cbq_handler(
        act_edit_command_notification,
        lambda c: c.data.startswith(f'{CBT.EDIT_CMD_NOTIFICATION_TEXT}:'),
    )
    tg.msg_handler(
        edit_command_notification,
        func=lambda m: tg.check_state(
            m.chat.id, m.from_user.id, CBT.EDIT_CMD_NOTIFICATION_TEXT
        ),
    )

    tg.cbq_handler(
        switch_notification,
        lambda c: c.data.startswith(f'{CBT.SWITCH_CMD_NOTIFICATION}:'),
    )
