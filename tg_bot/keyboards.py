...
from tg_bot.utils import (NotificationTypes, add_navigation_buttons,
                          bool_to_text)
...
def power_off(instance_id: int, state: int) -> K:
    ...
    if state == 0:
        kb.row(
            B(_('gl_yes'), None, f'{CBT.SHUT_DOWN}:1:{instance_id}'),
        ...
        buttons = [
            *[no_button] * (yes_button_num - 1),
            yes_button,
            *[no_button] * (10 - yes_button_num),
        ]
        ...
        buttons = [
            *[yes_button] * (yes_button_num - 1),
            no_button,
            *[yes_button] * (40 - yes_button_num),
        ]
...
def settings_sections(c: Vertex) -> K:
    ...
    kb.row(*lang_buttons).add(
        B(_('mm_global'), callback_data=f'{CBT.CATEGORY}:main')
    ).add(B(_('mm_notifications'), callback_data=f'{CBT.CATEGORY}:tg')).add(
        ...
        B(_('gl_next'), callback_data=CBT.MAIN2)
    )
    return kb
...
def main_settings(c: Vertex) -> K:
    ...
    kb = (
        K()
        .row(
            B(_('gs_autoraise', l('autoRaise')), None, f'{p}:autoRaise'),
            B(
                _('gs_autoresponse', l('autoResponse')),
                None,
                f'{p}:autoResponse',
            ),
        )
        .row(
            B(
                _('gs_autodelivery', l('autoDelivery')),
                None,
                f'{p}:autoDelivery',
            ),
            B(
                _('gs_nultidelivery', l('multiDelivery')),
                None,
                f'{p}:multiDelivery',
            ),
        )
        ...
        .row(
            B(
                _('gs_old_msg_mode', l('oldMsgGetMode')),
                None,
                f'{p}:oldMsgGetMode',
            ),
...
def new_message_view_settings(c: Vertex) -> K:
    ...
    kb = (
        ...
        .add(
            B(
                _('mv_incl_my_msg', l('includeMyMessages')),
                None,
                f'{p}:includeMyMessages',
            )
        )
        .add(
            B(
                _('mv_incl_fp_msg', l('includeFPMessages')),
                None,
                f'{p}:includeFPMessages',
            )
        )
        .add(
            B(
                _('mv_incl_bot_msg', l('includeBotMessages')),
                None,
                f'{p}:includeBotMessages',
            )
        )
        .add(
            B(
                _('mv_only_my_msg', l('notifyOnlyMyMessages')),
                None,
                f'{p}:notifyOnlyMyMessages',
            )
        )
        .add(
            B(
                _('mv_only_fp_msg', l('notifyOnlyFPMessages')),
                None,
                f'{p}:notifyOnlyFPMessages',
            )
        )
        .add(
            B(
                _('mv_only_bot_msg', l('notifyOnlyBotMessages')),
                None,
                f'{p}:notifyOnlyBotMessages',
            )
        )
...
def greeting_settings(c: Vertex):
    ...
    kb = (
        ...
        .add(
            B(
                _('gr_greetings', l('sendGreetings')),
                None,
                f'{p}:sendGreetings',
            )
        )
        .add(
            B(
                _('gr_cache_init_chats', l('cacheInitChats')),
                None,
                f'{p}:cacheInitChats',
            )
        )
        .add(
            B(
                _('gr_ignore_sys_msgs', l('ignoreSystemMessages')),
                None,
                f'{p}:ignoreSystemMessages',
            )
        )
    ...
    kb = (
        ...
        .add(
            B(
                _(
                    'oc_send_reply',
                    bool_to_text(int(c.MAIN_CFG['OrderConfirm']['sendReply'])),
                ),
                None,
                f'{CBT.SWITCH}:OrderConfirm:sendReply',
            )
        )
...
def review_reply_settings(c: Vertex):
    ...
    for i in range(1, 6):
        kb.row(
            B(f"{'⭐' * i}", None, f'{CBT.SEND_REVIEW_REPLY_TEXT}:{i}'),
            B(
                f"{bool_to_text(int(c.MAIN_CFG['ReviewReply'][f'star{i}Reply']))}",
                None,
                f'{CBT.SWITCH}:ReviewReply:star{i}Reply',
            ),
...
def notifications_settings(c: Vertex, chat_id: int) -> K:
    ...
    kb = (
        K()
        .row(
            B(_('ns_new_msg', l(n.new_message)), None, f'{p}:{n.new_message}'),
            B(_('ns_cmd', l(n.command)), None, f'{p}:{n.command}'),
        )
        .row(
            B(_('ns_new_order', l(n.new_order)), None, f'{p}:{n.new_order}'),
            B(
                _('ns_order_confirmed', l(n.order_confirmed)),
                None,
                f'{p}:{n.order_confirmed}',
            ),
        )
        .row(
            B(
                _('ns_lot_activate', l(n.lots_restore)),
                None,
                f'{p}:{n.lots_restore}',
            ),
            B(
                _('ns_lot_deactivate', l(n.lots_deactivate)),
                None,
                f'{p}:{n.lots_deactivate}',
            ),
        )
        .row(
...
def blacklist_settings(c: Vertex) -> K:
    ...
    kb = (
        ...
        .add(
            B(
                _('bl_autodelivery', l('blockDelivery')),
                None,
                f'{p}:blockDelivery',
            )
        )
        .add(
            B(
                _('bl_autoresponse', l('blockResponse')),
                None,
                f'{p}:blockResponse',
            )
        )
        .add(
            B(
                _(
                    ...
                    l('blockNewMessageNotification'),
                ),
                None,
                f'{p}:blockNewMessageNotification',
            )
        )
        .add(
            B(
                _(
                    ...
                    l('blockNewOrderNotification'),
                ),
                None,
                f'{p}:blockNewOrderNotification',
            )
        )
        .add(
            B(
                _('bl_command_notifications', l('blockCommandNotification')),
                None,
                f'{p}:blockCommandNotification',
            )
        )
        .add(B(_('gl_back'), None, CBT.MAIN))
    )
...
def commands_list(c: Vertex, offset: int) -> K:
    ...
    """
    kb = K()
    commands = c.RAW_AR_CFG.sections()[
        offset : offset + MENU_CFG.AR_BTNS_AMOUNT
    ]
    ...
    kb.add(B(_('ar_to_ar'), None, f'{CBT.CATEGORY}:ar')).add(
        B(_('ar_to_mm'), None, CBT.MAIN)
    )
    return kb
    ...
    kb = (
        ...
        .add(
            B(
                _('ar_edit_response'),
                None,
                f'{CBT.EDIT_CMD_RESPONSE_TEXT}:{command_index}:{offset}',
            )
        )
        .add(
            B(
                _('ar_edit_notification'),
                None,
                f'{CBT.EDIT_CMD_NOTIFICATION_TEXT}:{command_index}:{offset}',
            )
        )
        .add(
            B(
                _(
                    'ar_notification',
                    bool_to_text(
                        command_obj.get('telegramNotification'), '🔔', '🔕'
                    ),
                ),
                None,
                f'{CBT.SWITCH_CMD_NOTIFICATION}:{command_index}:{offset}',
            )
        )
        .add(
            B(_('gl_delete'), None, f'{CBT.DEL_CMD}:{command_index}:{offset}')
        )
        .row(
            B(_('gl_back'), None, f'{CBT.CMD_LIST}:{offset}'),
            B(
                _('gl_refresh'),
                None,
                f'{CBT.EDIT_CMD}:{command_index}:{offset}',
            ),
        )
    )
    return kb
...
def products_files_list(offset: int) -> K:
    ...
    files = os.listdir('storage/products')[
        offset : offset + MENU_CFG.PF_BTNS_AMOUNT
    ]
    ...
    for index, name in enumerate(files):
        ...
        keyboard.add(
            B(
                f"{amount} {_('gl_pcs')}, {name}",
                None,
                f'{CBT.EDIT_PRODUCTS_FILE}:{offset + index}:{offset}',
            )
        )
    ...
    keyboard = add_navigation_buttons(
        ...
        len(files),
        len(os.listdir('storage/products')),
        CBT.PRODUCTS_FILES_LIST,
    )

    keyboard.add(B(_('ad_to_ad'), None, f'{CBT.CATEGORY}:ad')).add(
        B(_('ad_to_mm'), None, CBT.MAIN)
    )
    return keyboard


def products_file_edit(
    file_number: int, offset: int, confirmation: bool = False
) -> K:
    """
    ...
    keyboard = (
        ...
        .add(
            B(
                _('gf_download'),
                None,
                f'download_products_file:{file_number}:{offset}',
            )
        )
    )
    if not confirmation:
        keyboard.add(
            B(
                _('gl_delete'),
                None,
                f'del_products_file:{file_number}:{offset}',
            )
        )
    else:
        keyboard.row(
            B(
                _('gl_yes'),
                None,
                f'confirm_del_products_file:{file_number}:{offset}',
            ),
            B(
                _('gl_no'),
                None,
                f'{CBT.EDIT_PRODUCTS_FILE}:{file_number}:{offset}',
            ),
        )
    keyboard.row(
        B(_('gl_back'), None, f'{CBT.PRODUCTS_FILES_LIST}:{offset}'),
        B(
            _('gl_refresh'),
            None,
            f'{CBT.EDIT_PRODUCTS_FILE}:{file_number}:{offset}',
...
def lots_list(vertex: Vertex, offset: int) -> K:
    ...
    if not lots and offset != 0:
        offset = 0
        lots = vertex.AD_CFG.sections()[
            offset : offset + MENU_CFG.AD_BTNS_AMOUNT
        ]

    for index, lot in enumerate(lots):
        keyboard.add(
            B(lot, None, f'{CBT.EDIT_AD_LOT}:{offset + index}:{offset}')
        )
...
def funpay_lots_list(c: Vertex, offset: int):
    ...
    for index, lot in enumerate(lots):
        keyboard.add(
            B(
                lot.description,
                None,
                f'{CBT.ADD_AD_TO_LOT}:{offset + index}:{offset}',
            )
        )
    ...
    keyboard.row(
        B(_('fl_manual'), None, f'{CBT.ADD_AD_TO_LOT_MANUALLY}:{offset}'),
        B(_('gl_refresh'), None, f'update_funpay_lots:{offset}'),
    ).add(B(_('ad_to_ad'), None, f'{CBT.CATEGORY}:ad')).add(
        B(_('ad_to_mm'), None, CBT.MAIN)
    )
    return keyboard
...
def edit_lot(c: Vertex, lot_number: int, offset: int) -> K:
    ...
    kb = K().add(
        B(
            _('ea_edit_delivery_text'),
            None,
            f'{CBT.EDIT_LOT_DELIVERY_TEXT}:{lot_number}:{offset}',
        )
    )
    if not file_name:
        kb.add(
            B(
                _('ea_link_goods_file'),
                None,
                f'{CBT.BIND_PRODUCTS_FILE}:{lot_number}:{offset}',
            )
        )
    else:
    ...
    p = {
        ...
        'md': (
            c.MAIN_CFG['FunPay'].getboolean('multiDelivery'),
            'disableMultiDelivery',
        ),
        'ares': (
            c.MAIN_CFG['FunPay'].getboolean('autoRestore'),
            'disableAutoRestore',
        ),
        'adis': (
            c.MAIN_CFG['FunPay'].getboolean('autoDisable'),
            'disableAutoDisable',
        ),
    }
    ...
    def l(s):
        return (
            '⚪' if not p[s][0] else '🔴' if lot_obj.getboolean(p[s][1]) else '🟢'
        )
    ...
    kb.row(
        B(
            _('ea_delivery', l('ad')),
            None,
            f"{f'{sl}:disable:{info}' if p['ad'][0] else dis}",
        ),
        B(
            _('ea_multidelivery', l('md')),
            None,
            f"{f'{sl}:disableMultiDelivery:{info}' if p['md'][0] else dis}",
        ),
    ).row(
        B(
            _('ea_restore', l('ares')),
            None,
            f"{f'{sl}:disableAutoRestore:{info}' if p['ares'][0] else dis}",
        ),
        B(
            _('ea_deactivate', l('adis')),
            None,
            f"{f'{sl}:disableAutoDisable:{info}' if p['adis'][0] else dis}",
        ),
    ).row(
        B(_('ea_test'), None, f'test_auto_delivery:{info}'),
        B(_('gl_delete'), None, f'{CBT.DEL_AD_LOT}:{info}'),
    ).row(
        B(_('gl_back'), None, f'{CBT.AD_LOTS_LIST}:{offset}'),
        ...
        if confirmation:
            kb.row(
                B(
                    _('gl_yes'),
                    None,
                    f'{CBT.REFUND_CONFIRMED}:{order_id}:{node_id}:{username}',
                ),
                B(
                    _('gl_no'),
                    None,
                    f'{CBT.REFUND_CANCELLED}:{order_id}:{node_id}:{username}',
                ),
            )
        else:
            kb.add(
                B(
                    _('ord_refund'),
                    None,
                    f'{CBT.REQUEST_REFUND}:{order_id}:{node_id}:{username}',
                )
            )

    kb.add(B(_('ord_open'), url=f'https://funpay.com/orders/{order_id}/')).row(
        B(
            _('ord_answer'),
            None,
...
def reply(
    node_id: int, username: str, again: bool = False, extend: bool = False
) -> K:
    """
    ...
    bts = [
        B(
            _('msg_reply2') if again else _('msg_reply'),
            None,
            f'{CBT.SEND_FP_MESSAGE}:{node_id}:{username}',
        ),
        B(
    ...
    if extend:
        bts.append(
            B(_('msg_more'), None, f'{CBT.EXTEND_CHAT}:{node_id}:{username}')
        )
    bts.append(
        B(f'🌐 {username}', url=f'https://funpay.com/chat/?node={node_id}')
    )
    kb = K().row(*bts)
    return kb
...
def templates_list(c: Vertex, offset: int) -> K:
    ...
    kb.add(B(_('tmplt_add'), None, f'{CBT.ADD_TMPLT}:{offset}')).add(
        B(_('gl_back'), None, CBT.MAIN)
    )
    return kb
    ...
    kb = (
        ...
        .add(
            B(
                _('gl_delete'),
                None,
                f'{CBT.DEL_TMPLT}:{template_index}:{offset}',
            )
        )
...
def templates_list_ans_mode(
    ...
    for index, tmplt in enumerate(templates):
        kb.add(
            B(
                tmplt.replace('$username', username),
                None,
                f'{CBT.SEND_TMPLT}:{offset + index}:{node_id}:{username}:{prev_page}{extra_str}',
            )
        )
    ...
    if prev_page == 0:
        kb.add(
            B(
                _('gl_back'),
                None,
                f'{CBT.BACK_TO_REPLY_KB}:{node_id}:{username}:0{extra_str}',
            )
        )
    elif prev_page == 1:
        kb.add(
            B(
                _('gl_back'),
                None,
                f'{CBT.BACK_TO_REPLY_KB}:{node_id}:{username}:1{extra_str}',
            )
        )
    elif prev_page == 2:
        kb.add(
            B(
                _('gl_back'),
                None,
                f'{CBT.BACK_TO_ORDER_KB}:{node_id}:{username}{extra_str}',
            )
        )
    return kb
