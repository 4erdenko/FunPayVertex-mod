...
def AD_SETTINGS() -> K:
    return (
        ...
        .add(
            B(
                _('ad_edit_goods_file'),
                callback_data=f'{CBT.PRODUCTS_FILES_LIST}:0',
            )
        )
        .row(
            B(
                _('ad_upload_goods_file'),
                callback_data=CBT.UPLOAD_PRODUCTS_FILE,
            ),
            B(
                _('ad_create_goods_file'),
...
def CONFIGS_UPLOADER() -> K:
    return (
        ...
        .add(
            B(
                _('cfg_download_ar'),
                callback_data=f'{CBT.DOWNLOAD_CFG}:autoResponse',
            )
        )
        .add(
            B(
                _('cfg_download_ad'),
                callback_data=f'{CBT.DOWNLOAD_CFG}:autoDelivery',
            )
        )
