...
class Account:
    ...
    def get(self, update_phpsessid: bool = False) -> Account:
        ...
        self.active_purchases = (
            int(active_purchases.text) if active_purchases else 0
        )
    ...
    def get_subcategory_public_lots(
        self, subcategory_type: enums.SubCategoryTypes, subcategory_id: int
        ...
        meth = (
            f'lots/{subcategory_id}/'
            if subcategory_type is enums.SubCategoryTypes.COMMON
            else f'chips/{subcategory_id}/'
        )
        response = self.method(
            'get', meth, {'accept': '*/*'}, {}, raise_not_200=True
        )
        html_response = response.content.decode()
        parser = BeautifulSoup(html_response, 'html.parser')
        ...
        subcategory_obj = self.get_subcategory(
            subcategory_type, subcategory_id
        )
        result = []
        for offer in offers:
            ...
            if subcategory_type is types.SubCategoryTypes.COMMON:
                price = float(
                    offer.find('div', {'class': 'tc-price'})['data-s']
                )
            else:
                price = float(
                    offer.find('div', {'class': 'tc-price'})
                    .find('div')
                    .text.split()[0]
                )
    ...
    def get_chat_history(
        ...
        if not json_response.get('chat') or not json_response['chat'].get(
            'messages'
        ):
            return []
        if isinstance(chat_id, int):
            interlocutor_id = int(
                json_response['chat']['node']['name'].split('-')[2]
            )
        else:
            interlocutor_id = None
        return self.__parse_messages(
    ...
    def get_chats_histories(
        self, chats_data: dict[int | str, str | None]
    ) -> dict[int, list[types.Message]]:
        """
        ...
        for i in json_response['objects']:
            ...
            messages = self.__parse_messages(
                i['data']['messages'],
                i.get('id'),
                interlocutor_id,
                interlocutor_name,
    ...
    def send_message(
        ...
        if image_id is not None:
            ...
            request['data']['content'] = (
                f'{self.__bot_character}{text}' if text else ''
            )
        ...
        try:
            if image_link := parser.find('a', {'class': 'chat-img-link'}):
                ...
                ).text.replace(self.__bot_character, '', 1)
    ...
    def refund(self, order_id):
        ...
        if response.json().get('error'):
            raise exceptions.RefundError(
                response, response.json().get('msg'), order_id
            )
    ...
    def raise_lots(
        ...
        if subcategories:
            ...
            subcats = [
                ...
                if i.type is types.SubCategoryTypes.COMMON
                and i.id not in exclude
            ]
        ...
        if not json_response.get('error'):
            ...
            and 'Подождите' in json_response.get('msg')
        ):
        ...
        for i in subcategories_divs:
            ...
            for j in offers:
                ...
                if subcategory_obj.type is types.SubCategoryTypes.COMMON:
                    price = float(
                        j.find('div', {'class': 'tc-price'})['data-s']
                    )
                else:
                    price = float(
                        j.find('div', {'class': 'tc-price'})
                        .find('div')
                        .text.split(' ')[0]
                    )
    ...
    def get_chat(self, chat_id: int) -> types.Chat:
        ...
        if (
            ...
            .find('div', {'class': 'media-user-name'})
            .find('a')
            .text
        ) == 'Чат':
    ...
    def get_order(self, order_id: str) -> types.Order:
        ...
        headers = {'accept': '*/*'}
        response = self.method(
            'get', f'orders/{order_id}/', headers, {}, raise_not_200=True
        )
        html_response = response.content.decode()
        parser = BeautifulSoup(html_response, 'html.parser')
        username = parser.find('div', {'class': 'user-link-name'})
        ...
        if (
            span := parser.find('span', {'class': 'text-warning'})
        ) and span.text == 'Возврат':
            status = types.OrderStatuses.REFUNDED
        elif (
            span := parser.find('span', {'class': 'text-success'})
        ) and span.text == 'Закрыт':
        ...
        if not (stars_obj := review_obj.find('div', {'class': 'rating'})):
            ...
            stars = int(
                stars_obj.find('div').get('class')[0].split('rating')[1]
            )
            text = review_obj.find(
                'div', {'class': 'review-item-text'}
            ).text.strip()

        if not (
            reply_obj := review_obj.find(
    ...
    def get_sells(
        ...
        **more_filters,
        ...
        parser = BeautifulSoup(html_response, 'html.parser')
        check_user = parser.find(
            'div', {'class': 'content-account content-account-login'}
        )
        if check_user:
            raise exceptions.UnauthorizedError(response)

        next_order_id = parser.find(
        ...
        for div in order_divs:
            ...
            description = (
                div.find('div', {'class': 'order-desc'}).find('div').text
            )
            price = float(
                div.find('div', {'class': 'tc-price'}).text.split(' ')[0]
            )

            buyer_div = div.find('div', {'class': 'media-user-name'}).find(
                'span'
            )
            buyer_username = buyer_div.text
            buyer_id = int(
                buyer_div.get('data-href')[:-1].split(
            ...
            if 'сегодня' in order_date_text:  # сегодня, ЧЧ:ММ
                ...
                order_date = datetime(
                    now.year, now.month, now.day, int(h), int(m)
                )
            elif 'вчера' in order_date_text:  # вчера, ЧЧ:ММ
                h, m = order_date_text.split(', ')[1].split(':')
                temp = now - timedelta(days=1)
                order_date = datetime(
                    temp.year, temp.month, temp.day, int(h), int(m)
                )
    ...
    def request_chats(self) -> list[types.ChatShortcut]:
        ...
        for msg in chats:
            ...
            chat_obj = types.ChatShortcut(
                chat_id, chat_with, last_msg_text, unread, str(msg)
            )
    ...
    def save_lot(self, lot_fields: types.LotFields):
        ...
        response = self.method(
            'post', 'lots/offerSave', headers, fields, raise_not_200=True
        )
        json_response = response.json()
        if json_response.get('error'):
            raise exceptions.LotSavingError(
                response, json_response.get('error'), lot_fields.lot_id
            )
    ...
    def __setup_categories(self, html: str):
        ...
        games_table = (
            games_table[1] if len(games_table) > 1 else games_table[0]
        )
        ...
        for i in games_divs:
            ...
            gname = i.find('a').text
            regional_games = {gid: types.Category(gid, gname)}
            if regional_divs := i.find('div', {'role': 'group'}):
                for btn in regional_divs.find_all('button'):
                    ...
                    regional_games[regional_game_id] = types.Category(
                        regional_game_id, f'{gname} ({btn.text})'
                    )
            ...
            for j in subcategories_divs:
                ...
                for k in subcategories:
                    ...
                    sobj = types.SubCategory(
                        sid, name, stype, regional_games[j_game_id]
                    )
    ...
    def __parse_messages(
        ...
        for i in json_messages:
            ...
            if self.chat_id_private and (
                image_link := parser.find('a', {'class': 'chat-img-link'})
            ):
                image_link = image_link.get('href')
                message_text = None
            else:
        ...
        for i in messages:
            ...
            i.badge = (
                badges.get(i.author_id)
                if badges.get(i.author_id) != 0
                else None
            )
        return messages
    ...
    def chat_id_private(chat_id: int | str):
        return isinstance(chat_id, int) or PRIVATE_CHAT_ID_RE.fullmatch(
            chat_id
        )
