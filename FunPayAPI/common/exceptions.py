...
class RaiseError(RequestFailedError):
    ...
    def short_str(self):
        return (
            f'Не удалось поднять лоты категории "{self.category.name}"'
...
class MessageNotDeliveredError(RequestFailedError):
    ...
    def short_str(self):
        return (
            f'Не удалось отправить сообщение в чат {self.chat_id}'
            f"{f': {self.error_message}' if self.error_message else '.'}"
        )
...
class FeedbackEditingError(RequestFailedError):
    ...
    def short_str(self):
        return (
            f'Не удалось изменить состояние отзыва / ответа на отзыв на заказ {self.order_id}'
            f"{f': {self.error_message}' if self.error_message else '.'}"
        )
...
class LotSavingError(RequestFailedError):
    ...
    def short_str(self):
        return (
            f'Не удалось сохранить лот {self.lot_id}'
            f"{f': {self.error_message}' if self.error_message else '.'}"
        )
...
class RefundError(RequestFailedError):
    ...
    def short_str(self):
        return (
            f'Не удалось вернуть средства по заказу {self.order_id}'
            f"{f': {self.error_message}' if self.error_message else '.'}"
        )
