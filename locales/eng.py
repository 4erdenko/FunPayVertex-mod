...
param_disabled = (
    '❌ This parameter is disabled globally and cannot be changed for this lot.\n\n'
...
create_test_ad_key = (
    'Enter the name of the lot whose auto-delivery you want to test.'
)
...
update_backup_created = (
    '✅ Backed up configs and storage in <code>backup.zip</code>.'
)
...
update_done = (
    '✅ The update is installed! Restart the FPV with the /restart command.'
)
update_done_exe = (
    '✅ The update is installed! New <code>FPV.exe</code> is in <code>update</code> folder. '
...
power_off_3 = (
    "Not that I'm imposing, but if you want to apply changes to the main config, "
    'you can just restart me with the /restart command.'
)
power_off_4 = (
    "Do you even read my messages? Let's put you to the test: yes = no, no = yes."
...
power_off_error = (
    '❌ This button does not belong to this session.\nCall this menu again.'
)

enter_msg_text = 'Enter message text.'
msg_sent = (
    '✅ Message sent to <a href="https://funpay.com/chat/?node={}">{}</a> chat.'
)
...
order_confirm_changed = (
    '✅ The text of the order confirmation reply has been changed!'
)
...
ad_already_ad_err = (
    '❌ Lot <code>{}</code> is already set up for auto-delivery.'
)
ad_lot_already_exists = (
    '❌ The lot <code>{}</code> already has auto-delivery linked to it.'
)
ad_lot_linked = '✅ Auto-delivery linked to lot <code>{}</code>.'
ad_link_gf = (
    'Enter the goods file name.\nSend <code>-</code>, if you want to unlink the goods file.\n\n'
...
ad_updating_lots_list = (
    'Updating lots and categories data (it may take some time)...'
)
...
gf_creation_err = (
    '❌ An error occurred while creating the <code>{}</code> file.'
)
...
gf_new_goods = (
    '✅ <code>{}</code> item(s) added to <code>storage/products/{}</code>.'
)
gf_empty_error = '❌ File storage/products/{} is empty.'
gf_linked_err = (
    '❌ File <code>storage/products/{}</code> is linked to one ore more lots.\n'
...
ar_subcmd_duplicate_err = (
    '❌ The command <code>{}</code> is duplicated in the command net.'
)
ar_cmd_already_exists_err = '❌ The command <code>{}</code> already exists.'
ar_enter_new_cmd = (
    'Enter a new command (or several commands via <code>|</code>).'
)
...
desc_ad_fp_lot_list = (
    "This is a list of lots from your FunPay profile. Select the lot you're interested in to link auto-delivery to it.\n"
...
desc_pl = (
    'Here you can get information about the plugins, as well as configure them.\n\n'
...
v_edit_order_confirm_text = (
    'Enter the text of the order confirmation response.'
)
...
v_date_text = (
    '<code>$date_text</code> - current date in <i>January 1</i> format.'
)
...
v_full_time = (
    '<code>$full_time</code> - current time in <i>HH:MM:SS</i> format.'
)
v_photo = (
    '<code>$photo=[PHOTO ID]</code> - photo. Instead of <code>PHOTO ID</code>, '
...
exc_not_enough_items = (
    'Not enough items in goods file "{}". Requested: {}, available: {}.'
)
...
exc_plugin_field_not_found = (
    'Failed to load the plugin "{}": required field "{}" does not exists.'
)

# Logs
log_tg_initialized = '$MAGENTATelegram bot initialized.'
log_tg_started = '$CYANTelegram bot $YELLOW@{}$CYAN started.'
log_tg_handler_error = (
    'An error occurred while executing the Telegram bot handler.'
)
...
log_watermark_deleted = (
    '$MAGENTA@{} (ID: {})$RESET deleted the message watermark.'
)
...
log_gf_unlinked = (
    '$MAGENTA@{} (ID: {})$RESET unlined goods file from $YELLOW{}$RESET.'
)
...
log_ar_cmd_deleted = (
    '$MAGENTA@{} (ID: {})$RESET deleted the command $YELLOW{}$RESET.'
)
log_cfg_downloaded = (
    '$MAGENTA@{} (ID: {})$RESET requested config $YELLOW{}$RESET.'
)
log_tmplt_added = (
...
log_pl_activated = (
    '$MAGENTA@{} (ID: {})$RESET activated the plugin $YELLOW"{}"$RESET.'
)
log_pl_deactivated = (
    '$MAGENTA@{} (ID: {})$RESET deactivated the plugin $YELLOW"{}"$RESET.'
)
log_pl_deleted = (
    '$MAGENTA@{} (ID: {})$RESET deleted the plugin $YELLOW"{}"$RESET.'
)
...
ntfc_new_order_will_be_delivered = (
    'ℹ️ The product will be delivered as soon as possible.'
)
...
crd_profile_get_timeout_err = (
    'Failed to load account lots data: timeout exceeded.'
)
...
crd_msg_send_err = (
    'An error occurred when sending a message to chat $YELLOW{}$RESET.'
)
...
crd_handlers_registered = (
    'The handlers from $YELLOW{}.py$RESET are registered.'
)
