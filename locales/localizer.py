...
class Localizer:
    ...
    def __init__(self, curr_lang: str | None = None):
        self.languages = {'ru': ru, 'eng': eng}
        self.default_language = 'ru'
        self.current_language = (
            curr_lang if curr_lang in self.languages else self.default_language
        )
    ...
    def translate(self, variable_name: str, *args):
        ...
        if not hasattr(self.languages[self.current_language], variable_name):
            if not hasattr(
                self.languages[self.default_language], variable_name
            ):
            ...
            text = getattr(
                self.languages[self.current_language], variable_name
            )
