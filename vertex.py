...
from tg_bot import (auto_delivery_cp, auto_response_cp, config_loader_cp,
                    file_uploader, templates_cp)
...
def check_proxy(proxy: dict) -> bool:
    ...
    try:
        response = requests.get(
            'https://api.ipify.org/', proxies=proxy, timeout=10.0
        )
    except:
...
class Vertex(object):
    ...
    def __init__(
        ...
        self.delivery_tests = (
            {}
        )  # Одноразовые ключи для тестов автовыдачи. {"ключ": "название лота"}
        ...
        if self.MAIN_CFG['Proxy'].getboolean('enable'):
            if (
                self.MAIN_CFG['Proxy']['ip']
                and self.MAIN_CFG['Proxy']['port'].isnumeric()
            ):
                logger.info(_('crd_proxy_detected'))

                ip, port = (
                    self.MAIN_CFG['Proxy']['ip'],
                    self.MAIN_CFG['Proxy']['port'],
                )
                login, password = (
                    self.MAIN_CFG['Proxy']['login'],
                    self.MAIN_CFG['Proxy']['password'],
                )
                self.proxy = {
                ...
                if self.MAIN_CFG['Proxy'].getboolean(
                    'check'
                ) and not check_proxy(self.proxy):
                    sys.exit()
        ...
        self.raise_time = (
            {}
        )  # Временные метки поднятия категорий {id игры: след. время поднятия}
        self.profile: FunPayAPI.types.UserProfile | None = (
            None  # FunPay профиль для всего вертекса (+ хэндлеров)
        )
        self.tg_profile: FunPayAPI.types.UserProfile | None = (
            None  # FunPay профиль (для Telegram-ПУ)
        )
        self.last_tg_profile_update = (
            datetime.datetime.now()
        )  # Последнее время обновления профиля для TG-ПУ
        self.curr_profile: FunPayAPI.types.UserProfile | None = (
            None  # Текущий профиль (для восст. / деакт. лотов.)
        )
        ...
        self.old_users = (
            vertex_tools.load_old_users()
        )  # Уже написавшие пользователи.
        ...
        for subcat in self.profile.get_sorted_lots(2):
            ...
            try:
                ...
                if e.wait_time is not None:
                    ...
                    logger.error(
                        _('crd_raise_unexpected_err', subcat.category.name)
                    )
                ...
                if (
                    isinstance(e, FunPayAPI.exceptions.RequestFailedError)
                    and e.status_code == 429
                ):
                    logger.warning(
                        _('crd_raise_429_err', subcat.category.name)
                    )
                    time.sleep(10)
            ...
            logger.info(
                _('crd_raise_wait_3600', vertex_tools.time_to_str(3600))
            )
            next_time = int(time.time()) + 3600
            self.raise_time[subcat.category.id] = next_time
            next_call = next_time if next_time < next_call else next_call
            self.run_handlers(
                self.post_lots_raise_handlers, (self, subcat.category)
            )
    ...
    def send_message(
        ...
        """
        if (
            self.MAIN_CFG['Other'].get('watermark')
            and watermark
            and not message_text.strip().startswith('$photo=')
        ):
            message_text = (
                f"{self.MAIN_CFG['Other']['watermark']}\n" + message_text
            )
    ...
    def update_lots_and_categories(self):
        ...
        """
        result = self.__update_profile(
            infinite_polling=False, attempts=3, update_main_profile=False
        )
        return result

    def switch_msg_get_mode(self):
        self.MAIN_CFG['FunPay']['oldMsgGetMode'] = str(
            int(not self.old_mode_enabled)
        )
        self.save_config(self.MAIN_CFG, 'configs/_main.cfg')
        if not self.runner:
            return
        self.runner.make_msg_requests = (
            False if self.old_mode_enabled else True
        )
    ...
    def run_handlers(self, handlers_list: list[Callable], args) -> None:
        ...
        for func in handlers_list:
            try:
                if (
                    getattr(func, 'plugin_uuid') is None
                    or self.plugins[getattr(func, 'plugin_uuid')].enabled
                ):
                    func(*args)
            except:
    ...
    def toggle_plugin(self, uuid):
        ...
        if self.plugins[uuid].enabled and uuid in self.disabled_plugins:
            ...
            not self.plugins[uuid].enabled
            and uuid not in self.disabled_plugins
        ):
    ...
    def bl_msg_notification_enabled(self) -> bool:
        return self.MAIN_CFG['BlockList'].getboolean(
    ...
    def only_my_msg_enabled(self) -> bool:
        return self.MAIN_CFG['NewMessageView'].getboolean(
