...
from FunPayAPI.types import Order, OrderShortcut
from FunPayAPI.updater.events import *
from locales.localizer import Localizer
from tg_bot import keyboards, utils
...
def save_init_chats_handler(c: Vertex, e: InitialChatEvent):
    ...
    """
    if (
        c.MAIN_CFG['Greetings'].getboolean('cacheInitChats')
...
def log_msg_handler(c: Vertex, e: NewMessageEvent):
    ...
    for index, event in enumerate(e.stack.get_stack()):
        username, text = (
            ...
            event.message.text or event.message.image_link,
        )
...
def greetings_handler(
    c: Vertex, e: NewMessageEvent | LastChatMessageChangedEvent
):
    """
    ...
    if not c.old_mode_enabled:
        if isinstance(e, LastChatMessageChangedEvent):
            return
        obj = e.message
        chat_id, chat_name, mtype, its_me = (
            obj.chat_id,
            obj.chat_name,
            obj.type,
    ...
    text = vertex_tools.format_msg_text(
        c.MAIN_CFG['Greetings']['greetingsText'], obj
    )
    Thread(
        target=c.send_message, args=(chat_id, text, chat_name), daemon=True
    ).start()


def add_old_user_handler(
    c: Vertex, e: NewMessageEvent | LastChatMessageChangedEvent
):
    """
...
def send_response_handler(
    ...
    response_text = vertex_tools.format_msg_text(
        c.AR_CFG[command]['response'], obj
    )
    Thread(
        target=c.send_message,
        args=(chat_id, response_text, chat_name),
        daemon=True,
    ).start()


def old_send_new_msg_notification_handler(
    c: Vertex, e: LastChatMessageChangedEvent
):
    if any(
        [
...
def send_new_msg_notification_handler(c: Vertex, e: NewMessageEvent) -> None:
    ...
    for i in events:
        ...
        if (
            i.message.author_id == last_message_author_id
            and i.message.by_bot == last_by_bot
        ):
            author = ''
        elif i.message.author_id == c.account.id:
            author = (
                f"<i><b>🤖 {_('you')} (<i>FPV</i>):</b></i> "
                if i.message.by_bot
            ...
            author = f"<i><b>🆘 {i.message.author} {_('support')}: </b></i>"
        msg_text = (
            f'<code>{i.message}</code>'
            if i.message.text
            else f"<a href=\"{i.message}\">{_('photo')}</a>"
        )
...
def send_review_notification(
    c: Vertex, order: Order, chat_id: int, reply_text: str | None
):
    if not c.telegram:
        return
    reply_text = (
        f'\n\n🗨️<b>Ответ:</b> \n<code>{utils.escape(reply_text)}</code>'
        if reply_text
        else ''
    )
    Thread(
        target=c.telegram.send_notification,
        args=(
            f"🔮 Вы получили {'⭐' * order.review.stars} за заказ <code>{order.id}</code>!\n\n"
...
def process_review_handler(
    ...
    if not c.old_mode_enabled:
        if isinstance(e, LastChatMessageChangedEvent):
            return
        message_type, its_me = e.message.type, c.account.username in str(
            e.message
        )
        ...
        (
            message_type,
            its_me,
        ) = e.chat.last_message_type, c.account.username in str(e.chat)
    ...
    def send_reply():
        ...
        try:
            ...
            logger.error(
                f'Не удалось получить информацию о заказе #{order_id}.'
            )
        ...
        if c.MAIN_CFG['ReviewReply'].getboolean(toggle) and c.MAIN_CFG[
            ...
            try:
                reply_text = vertex_tools.format_order_text(
                    c.MAIN_CFG['ReviewReply'].get(text), order
                )
                c.account.send_review(order_id, reply_text)
            except:
                logger.error(
                    f'Произошла ошибка при ответе на отзыв {order_id}.'
                )
                logger.debug('TRACEBACK', exc_info=True)
...
def send_command_notification_handler(
    ...
    if command not in c.AR_CFG or not c.AR_CFG[command].getboolean(
...
def test_auto_delivery_handler(
    ...
    html = (
        ORDER_HTML_TEMPLATE.replace('$username', chat_name)
...
def send_new_order_notification_handler(c: Vertex, e: NewOrderEvent, *args):
    ...
    if not c.telegram:
        return
    if e.order.buyer_username in c.blacklist and c.MAIN_CFG[
        'BlockList'
    ].getboolean('blockNewOrderNotification'):
        return
    ...
    text = _(
        'ntfc_new_order',
        utils.escape(e.order.description),
...
def deliver_goods(c: Vertex, e: NewOrderEvent, *args):
    ...
    delivery_text = vertex_tools.format_order_text(
        cfg_obj['response'], e.order
    )

    amount, goods_left, products = 1, -1, []
    try:
        if file_name := cfg_obj.get('productsFileName'):
            if c.multidelivery_enabled and not cfg_obj.getboolean(
            ...
            products, goods_left = vertex_tools.get_products(
    ...
    if not result:
        ...
        if file_name and products:
            vertex_tools.add_products(
                f'storage/products/{file_name}',
                products,
...
def deliver_product_handler(c: Vertex, e: NewOrderEvent, *args) -> None:
    ...
    if e.order.buyer_username in c.blacklist and c.MAIN_CFG[
        'BlockList'
    ].getboolean('blockDelivery'):
        logger.info(
            f'Пользователь {e.order.buyer_username} находится в ЧС и включена блокировка автовыдачи. '
...
def update_lot_state(
    vertex: Vertex, lot: types.LotShortcut, task: int
) -> bool:
    """
    ...
    while attempts:
        try:
            ...
            if task == 1:
                ...
                vertex.account.save_lot(lot_fields)
                logger.info(
                    f'Деактивировал лот $YELLOW{lot.description}$RESET.'
                )
            return True
        except Exception as e:
            if (
                isinstance(e, exceptions.RequestFailedError)
...
def update_lots_states(vertex: Vertex, event: NewOrderEvent):
    if not any([vertex.autorestore_enabled, vertex.autodisable_enabled]):
        return
    if (
        vertex.curr_profile_last_tag != event.runner_tag
        or vertex.last_state_change_tag == event.runner_tag
    ):
        return
    ...
    for lot in vertex.profile.get_lots():
        ...
        if lot.id not in lots:
            ...
            # и найден в конфиге автовыдачи
                ...
                if vertex.autorestore_enabled and config_obj.get(
                    'disableAutoRestore'
                ) in ['0', None]:
        ...
        # Если же лот активен
            ...
            if config_obj:
                ...
                if all(
                    (
                        not products_count,
                        vertex.MAIN_CFG['FunPay'].getboolean('autoDisable'),
...
<code>{lots}</code>"""
        Thread(
            target=vertex.telegram.send_notification,
            args=(text,),
            kwargs={
...
def update_lots_state_handler(vertex: Vertex, event: NewOrderEvent, *args):
    Thread(
        target=update_lots_states, args=(vertex, event), daemon=True
    ).start()
...
def send_thank_u_message_handler(c: Vertex, e: OrderStatusChangedEvent):
    ...
    """
    if (
        not c.MAIN_CFG['OrderConfirm'].getboolean('sendReply')
        or e.order.status is not types.OrderStatuses.CLOSED
    ):
        return

    text = vertex_tools.format_order_text(
        c.MAIN_CFG['OrderConfirm']['replyText'], e.order
    )
    chat = c.account.get_chat_by_name(e.order.buyer_username, True)
    logger.info(
        f'Пользователь $YELLOW{e.order.buyer_username}$RESET подтвердил выполнение заказа '
        f'$YELLOW{e.order.id}.$RESET'
    )
    logger.info(f'Отправляю ответное сообщение ...')
    Thread(
        target=c.send_message,
        args=(chat.id, text, e.order.buyer_username),
        daemon=True,
    ).start()


def send_order_confirmed_notification_handler(
    vertex: Vertex, event: OrderStatusChangedEvent
):
    """
    ...
    Thread(
        ...
        args=(
            ...
            keyboards.new_order(
                event.order.id, event.order.buyer_username, chat.id
            ),
